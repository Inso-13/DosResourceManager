; ******************************************************************************
; *                                                                            *
; * BGI-Treiber f�r Super-VGAs im 256-Farben Modus.                            *
; *                                                                            *
...
; Die Symbole werden normalerweise �ber's Makefile bzw. den TASM Aufruf
; gesetzt.
;
; P80286          = 1
; P80386          = 1
...
PROC    Emulate NEAR
        ...
        db      00h, 00h, 00h, 00h      ; call FAR GRAPH:XXXX �berschrieben
NOP_Vector:
        ret                             ; Danach gehts zur�ck
...
; die Autodetect-Routine aufrufen. R�ckgabe ist ein Eintrag der Mode-Tabelle
; in di

        mov     di, cx                          ; Modus-Nummer in di
        mov     al, [BYTE AutoMode-1+di]        ; Modus-BitSet holen
        push    es                              ; es wird zerst�rt
...
@@L5:   mov     ax, SIZE TMode                  ; Gr��e eines Tabelleneintrags
        mul     cx
        mov     di, ax                          ; Offset nach di
        lea     di, [ModeTable + di]            ; Adresse nach di
@@L6:   mov     [ModePtr], di                   ; Adresse merken

; Aus den Daten des Modus-Eintrags in di die Daten f�r die DST berechnen
; und setzen

        mov     ax, [(TMode di).XRes]           ; X-Aufl�sung setzen
        dec     ax
        mov     [DST.XRes], ax
        mov     [DST.XEfRes], ax
        mov     ax, [(TMode di).YRes]           ; Y-Aufl�sung setzen
        dec     ax
        mov     [DST.YRes], ax
        mov     [DST.YEfRes], ax
...
; Zeiger auf DST in es:bx zur�ckliefern, es stimmt schon

        mov     bx, OFFSET DST                  ; als aktuelle DST vermerken
        ret

IllegalMode:
        mov     di, OFFSET ModeTable            ; VGA256 r�ckliefern
        mov     [DST.Stat], grInvalidMode       ; Ung�ltiger Modus
...
; (GRAPH nimmt's anscheinend nicht so genau mit der Fehlerpr�fung, so da�
; ein Aufruf mit gesetztem Fehler durchaus m�glich ist. In diesem Fall ist
...
; ausgef�hrt, pr�ft das Grafik-Kernel ab Version 7.0 den FillPoly-Vektor.
...
; X- und Y-Aufl�sung und das Clip-Fenster r�cksetzen
...
; Die f�r den Modus passende Segment-Umschaltroutine setzen
...
; Hintergrundfarbe als Paletteneintrag 0 setzen
        ...
        mov     bl, [BkColor]
        xor     bh, bh
        mov     ax, 0C000h              ; Code f�r: Setze Hintergrund
...
ENDIF
        ...
        jcxz    @@L5
        mov     bx, di
        add     bx, cx                  ; Segment�berlauf?
        jnc     @@L4                    ; Springe wenn Nein
        jmp     @@L1                    ; Springe wenn Segment�berlauf

IF      P80386

@@L0:   mov     bx, di                  ; Rest nach Segment�berlauf
@@L1:   mov     cx, di
        neg     cx                      ; Rest bis Segment�berlauf
        jnz     @@L2
        mov     ecx, 4000h
        rep     stosd
        jmp     @@L3
...
; Wenn der Treiber im Protected-Mode l�uft und die VESA-Segmentumschaltung
; auch im Protected-Mode aufgerufen wurde, dann ist ein Deskriptor daf�r
...
; Den Emulate-Vektor wieder r�cksetzen: Wie bereits im Kommentar zur
; Vektor-Tabelle ausgef�hrt, pr�ft das Grafik-Kernel ab Version 7.0 den
; FillPoly-Vektor.
; Wenn dieser nicht auf Emulate steht setzt sich das Kernel ein Flag, das
; zur Folge hat, dass Arc und PieSlice mit kleinen Radien nicht mehr korrekt
...
; korrekt tut, muss er hier wieder r�ckgesetzt werden.
...
; =======================================================================
; Farbe setzen, al = Zeichenfarbe, ah = F�llfarbe
;
...
;               reservierten Speicherbereich (GraphBuf oder so), der auch f�r
;               FloodFill benutzt wird. Am Ende der Liste ist daher Platz f�r
...
;               mitgez�hlt werden.
;
; Die �bergebene Liste ist geschlossen, d.h. erster und letzter Punkt eines
...
; Variablen speichern bzw. initialisieren
        ...
        and     ax, 7FFFh               ; BP 7.0 (High Bit ist Flag f�r was ?)
...
; Die von Graph eingef�gten Trenn- und Abschlu�punkte werden nicht ben�tigt,
; sie werden beim Durchsuchen der Liste gel�scht.
...
; Der Punkt ist g�ltig, pr�fen ob er als Maximum oder Minimum in Frage kommt.
        ...
        xchg    ax, bx                  ; �bernehme wenn ja
        loopcx  @@L0                    ; Wert > Max kann nicht < Min sein
        jmp     @@C1

@@L1:   cmp     ax, dx                  ; Y-Wert < YMin ?
        jg      @@L2                    ; Springe wenn Nein
        xchg    ax, dx                  ; �bernehme wenn ja
@@L2:   loopcx  @@L0                    ; N�chster Punkt
        jmp     @@C1

; Der Punkt ist ung�ltig und wird beim Kopieren �bersprungen

@@L3:   inc     si
        inc     si                      ; Y-Wert ignorieren
        dec     [Count]                 ; Ein Punkt weniger
        jmp     @@L0                    ; und n�chster Punkt

; Alle Trenn- und Abschlu�punkte sind jetzt gel�scht, in bx befindet sich
; der gr��te und in dx der kleinste Y-Wert des Polygons. Die Anzahl der
; Punkte in Count entspricht der wirklichen Anzahl.
; Maximum und Minimum werden gespeichert und es wird ein Zeiger auf das
; Ende der Polygon-Liste berechnet, der Platz dahinter dient sp�ter zur
...
; F�r alle Y-Werte des Polygons werden jetzt die Start- und Endpunkte
; horizontaler Linien festgelegt, mit denen sp�ter das Polygon gef�llt wird.
; Die X-Werte werden dabei vorerst unsortiert in der Linienliste abgelegt
; und erst sp�ter aufsteigend sortiert.
...
; Die Steigung der Begrenzungslinie des Polygons ist 0 ! F�r die sp�tere
; Linie m�ssen also beide X-Koordinaten gespeichert werden. Es gibt also
...
; Endkoordinaten werden festgehalten.
        ...
        inc     [LineCount]             ; Anzahl der Punkte erh�hen
        mov     ax, [WORD ds:si+4]      ; X-Wert 2
        jmp     @@L8                    ; speichern und n�chste Linie
...
@@L12:  dec     dx
        ...
        mov     [WORD ds:si-2], ax      ; Kleinsten Wert r�ckspeichern
        loopcx  @@L10                   ; N�chster X-Wert

; Die Linienliste ist jetzt aufsteigend sortiert. F�r jeweils zwei
...
; Die Linie mu� noch in X-Richtung geclippt werden.
        ...
        call    [HorLineInit]           ; Initialisierung f�r HorLine durchf�hren
...
; Weiter mit dem n�chsten Y-Wert

@@L20:  inc     [Y]
        jmp     @@L4                    ; N�chster Y-Wert
...
;---------------------------------------------------
; Unterprogramm f�r Text-Ausgabe
; Input
;  di = Zeichen
; RETURN
...
ELSE
        shl     di, 1
        shl     di, 1
        shl     di, 1                   ; * 8 f�r Index bei 8x8 Zeichensatz
ENDIF
        cmp     di, 128 * 8             ; Code >= 128 ?
        jb      ROMCh                   ; Nein

; Zeiger es:di von 0000h:0007Ch holen (RAM-Zeichensatz f�r Zeichen ab 80h)
...
; Falls der Font um 90� gedreht werden mu� --> ab daf�r
...
; Wir m�ssen das Zeichen nur noch speichern
...
; Zuerst Zeichenposition gegen Clip-Fenster pr�fen
...
; Berechnen, wieviel am Ende einer Zeile (X2) dazugez�hlt werden mu� um bei
; (X1/Y+1) zu landen

        mov     ax, [MaxX]              ; Zeilenl�nge in Pixeln
        sub     ax, [TextSizeX]         ; - Zeichengr��e in Pixeln
...
; Variable laden
        ...
        mov     al, [DrawingColor]
        mov     es, [VideoSeg]
        mov     bx, [TextMultX]         ; Ins Register f�r Speed
        pop     di                      ; Offset
...
@@OC4:  mov     cx, bx                  ; X-Gr��e des Pixels
        rol     ah, 1                   ; Pixel setzen ?
        jc      @@OC5                   ; Ja

; Pixel nicht setzen, trotzdem di erh�hen

        add     di, cx
        jnc     @@OC7                   ; Kein Segment-�bertrag
        inc     [Seg64]                 ; �bertrag
        call    [SegSelect]             ; einstellen
        jmp     @@OC7                   ; N�chstes Zeichen-Pixel
...
@@OC9:  inc     [Seg64]                 ; N�chstes Segment
        call    [SegSelect]             ; ... einstellen
        jmp     @@OC6

@@OC10: inc     [Seg64]                 ; N�chstes Segment
        call    [SegSelect]
        jmp     @@OC8
...
@@OC5:  mov     [es:di], al             ; Pixel setzen
        inc     di                      ; N�chstes Pixel
        jz      @@OC9                   ; Segment-�berlauf behandeln
@@OC6:  loopcx  @@OC5

; N�chstes Zeichen-Pixel

@@OC7:  dec     dx
        jnz     @@OC4

; N�chste gleiche Zeile

        add     di, [Diff]              ; N�chste Bildschirmzeile
        jc      @@OC10                  ; �berlauf
@@OC8:  dec     [YMult]
        jnz     @@OC3
...
; Gef�hrlich ! Hier wird die X-Gr��e des Fonts wahlweise auf die X- oder Y-
...
; X und Y auf je 8 Pixel abrunden und daf�r sorgen, da� die Gr��e mindestens
; 1 betr�gt
...
; Jetzt die tats�chliche Gr��e r�ckliefern
...
; Errechnen wieviel bei Erreichen des Punktes X2+1 dazugez�hlt werden mu� um
...
; Nach bx die Anzahl der Zeilen holen
        ...
        mov     bx, [es:bx+2]           ; H�he - 1
        inc     bx

; Die Adresse des Pixels bei X1/Y1 berechnen. Danach enth�lt ax:si die aktuelle
...
; Testen, ob in der Zeile ein Segment-�berlauf auftreten kann

        mov     ax, si                  ; Bildschirm-Offset
        add     ax, cx                  ; + zu holende Pixel
        jc      Save4                   ; Ja, �berlauf

; Zeile hat keinen �berlauf
...
; X-Schleife wenn �berlauf

Even
Save4:  movsb                           ; Byte �bertragen
        or      si, si
        jz      Save6                   ; �bertrag
Save5:  loopcx  Save4
        jmp     Save2

; Segment-�berlauf in der X-Schleife korrigieren
...
; Segment-�berlauf in der Y-Schleife korrigieren
...
;
; Folgende Register d�rfen von den Unterprogrammen ver�ndert werden:
; AX, DI
;
; Die Unterprogramme, die eine Ver�nderung vornehmen sind hier als
; Macro codiert (OPC ist der zust�ndige Operand):
;

MACRO   RestoreOP OP

; Testen, ob in der Zeile ein �berlauf auftreten kann

        mov     ax, di                  ;; Offset
        add     ax, cx                  ;; + zu setzende Pixel
        jc      @@L4                    ;; �berlauf m�glich

; Zeile kann ohne �berlauf kopiert werden
...
@@L2:   lodsw                           ;; Wort holen
        ...
        inc     di                      ;; n�chstes Wort adressieren
        inc     di
        dec     cx
        jnz     @@L2
        ret
...
; Zeile kopieren wenn Segment-�berlauf
...
; Testen, ob in der Zeile ein �berlauf auftreten kann
        ...
        mov     ax, di                  ; Offset
        add     ax, cx                  ; + zu setzende Pixel
        jc      ROver1                  ; �berlauf m�glich

; Kein �berlauf m�glich
...
; Zeile kopieren wenn Segment-�berlauf in der Zeile

Even
ROver1: movsb                           ; Byte �bertragen
        or      di, di
        jz      ROver3                  ; Kein �bertrag
...
; Segment-�berlauf behandeln
; ACHTUNG: Dies mu� immer geschehen, auch dann wenn der Z�hler in cx abgelaufen
...
@@L2:   mov     ax, ds                  ; ds retten
        ...
        call    [SegSelect]
        mov     ds, ax                  ; altes ds
        jcxz    @@L4                    ; Nur f�r den Fall des Falles
        jmp     @@L1

; Byte nicht schreiben

EVEN
@@L3:   inc     di                      ; Byte �berspringen
        loopne  @@L1                    ; Schleife wenn kein Seg-�berlauf
        je      @@L2                    ; Springe wenn �berlauf
...
; Operation ausf�hrt. Breite holen und speichern
...
; Die Adresse des Pixels bei X1/Y1 berechnen. Danach enth�lt ax:di die aktuelle
...
; Bits 11, bx ist Hintergrundfarbe
        ...
        xor     bl,bl                   ; Index f�r Hintergrund ist 0
...
; Bits 10 --> RGB-Palette
; Zur Anpassung an das Handbuch m�ssen die Farbwerte noch jeweils um 2 Bit nach
; rechts geschoben werden, die untersten 2 Bit sind laut Handbuch nicht belegt,
; der DAC ben�tigt aber einen Bereich von 0..3Fh

; �nderung am 21.08.1991 laut c't 9/91 S. 162. Vor dem Setzen des Blau-Anteils
; wird auf den horizontalen Strahlr�cklauf gewartet um Zugriffskonflikte
...
; Warten auf den horizontalen Strahlr�cklauf
...
PROC    AllPalette      Near
        ...
        mov     al, 02h
        mov     dx, bx
        mov     ah, 16                  ; 16 St�ck hier nur wegen Kompatibilit�t
...
PROC    TextSize        Near
        ...
        xchg    ax, cx                  ; L�nge nach ax
        mul     [TextSizeX]             ; * Breite eines Zeichens
        xchg    bx, ax                  ; Resultat nach bx
        mov     cx, [TextSizeY]         ; cx ist H�he eines Zeichens
        ret
...
; Wartet aus Performance-Gr�nden nicht auf die GE
;
...
PROC    ColorQuery NEAR
        ...
        mov     cx, MaxColors-1         ; aber 256 zul�ssig
        ret
...
; Y-Werte um den Offset f�r die eingestellte Bildschirmseite korrigieren
...
; Errechnen wieviel bei Erreichen des Punktes X2+1 dazugez�hlt werden mu� um
; X1 in der n�chsten Linie zu erreichen
; Breite und H�he in Pixeln rechnen
...
; Das achtfache der Zeilenl�nge in Bytes zwischenspeichern. Alle 8 Zeilen
; wiederholt sich n�mlich das Muster, und (wenn zwischen den 8 Zeilen kein
; �berlauf auftritt) kann einfach mit rep movsb kopiert werden.
; Au�erdem mu� ein Z�hler f�r die Zeilen mitgef�hrt werden, da das nur geht,
; wenn mindestens 7 Zeilen geschrieben worden sind (sieht zwar auch sonst gut
; aus, erf�llt aber andere Kriterien ).

        mov     ax, [MaxX]              ; Zeilenl�nge in Bytes
IF P80286
        shl     ax, 3
ELSE
        mov     cl, 3
        shl     ax, cl                  ; * 8
ENDIF
        mov     [Lines8], ax            ; Bytes f�r 8 Zeilen
        mov     [Lines], 0000           ; Zeilenz�hler
...
; Y-Schleife
; Passendes Muster f�r den Y-Wert holen

Even
Bar1:   mov     cx, [BW]                ; Breite des Rechtecks = Anzahl Punkte

; Pr�fen, ob innerhalb der Zeile ein Segment-�berlauf auftreten kann. Wenn
; ja, dann F�llen mit Abpr�fen des Overflows und Muster per Hand.

        mov     dx, di                  ; Adress Offset
        add     dx, cx                  ; + zu setzende Pixel
        jc      Bar2

; In dieser Zeile tritt kein �berlauf auf.
; Als n�chstes pr�fen, ob bereits 7 Zeilen auf dem Bildschirm stehen. Wenn
; nicht --> F�llen zu Fu�
...
; selben Segment liegt k�nnen wir einfach kopieren.

        mov     si, di                  ;
        sub     si, [Lines8]            ; - Bytes f�r 8 Zeilen
        jc      Bar2                    ; �berlauf, war nix

; Sodele: F�llen durch Kopieren
...
Bar4:   inc     [BY1]                   ; N�chste Zeile
        add     di, [Diff]              ; Adresse weitersetzen
        jc      Bar04                   ; �bertrag auf Segment
Bar0:   inc     [Lines]                 ; Wir haben eine Zeile mehr
        dec     [BHe]                   ; H�he - 1
        jnz     Bar1

; Und hier ist das Ende. ds wird nicht r�ckgesetzt, weil der Dispatcher es
...
; Segment-�berlauf in der Y-Schleife behandeln

Bar04:  inc     [Seg64]                 ; �bertrag
        call    [SegSelect]             ; Segment einstellen
        jmp     Bar0                    ; Und weiter

; ----------------------------------------------------
; Zeile mit �berlauf oder zu wenig Zeilen zum Kopieren
; Passendes Muster f�r den Y-Wert holen
...
; Anzahl Punkte holen (neu weil cx zerst�rt worden ist)

        mov     cx, [BW]                ; Breite des Rechtecks = Anzahl Punkte

; Los geht's
; Muster testen (Even f�r Optimierung)
...
Bar5:   rol     ah, 1                   ; Punkt setzen ?
        ...
        jz      Bar6                    ; �berlauf korrigieren
Bar7:   loopcx  Bar5                    ; N�chster Punkt
        jmp     Bar4                    ; N�chste Linie

; Hierher wenn Punkt gesetzt wird

Even
Bar3:   mov     [es:di], bh             ; Punkt in F�llfarbe setzen
        inc     di                      ; N�chsten Punkt adressieren
        jz      Bar8                    ; �berlauf korigieren
Bar9:   loopcx  Bar5                    ; N�chster Punkt
        jmp     Bar4                    ; N�chste Linie

; Hier Einsprung bei Segment-�berl�ufen
...
; -------------------------------------------------------------
; Spezialfall f�r schnelles F�llen: Kein Muster vorhanden.
; Registerbelegung:
;   AL  = Farbe
...
PatBar1:
        mov     si, [BHe]               ; H�he des Rechtecks
        mov     bx, [Diff]              ; Wert f�r �berlauf von X2+1 --> X1
        mov     ah, al                  ; Farbe auch in ah
        mov     bp, [BW]                ; Ab hier keine lokalen Variablen mehr

PB1:    mov     cx, bp                  ; Breite des Rechtecks

; Testen ob innerhalb der Zeile ein Segment-�berlauf auftreten kann

        mov     dx, di                  ; Adress-Offset
        add     dx, cx                  ; + zu setzende Pixel
        jc      PB2                     ; �berlauf in der Zeile

; Zeile ist ohne �berlauf
...
PB4:    shr     cx, 1                   ; / 2
        ...
        rep     stosb                   ; �briges Byte falls cx <> 0
...
; Innerhalb der Zeile ist ein Segment-�berlauf

PB2:    mov     [es:di], al             ; Pixel setzen
        inc     di
        jnz     PB3                     ; kein Segment-�berlauf
        inc     [Seg64]                 ; �berlauf
...
; Routine ist X1 (di) immer positiv, die Adress-Berechnung verl�uft also ganz
; normal.
;
; Registerbelegung:
;    ax  = Y
...
; Den zweiten Punkt plotten
        ...
        add     di, si                  ; + DeltaX
        jc      @@L3                    ; Segment-�berlauf
        stosb                           ; Punkt setzen
        ret

; Segment-�berlauf korrigieren

@@L3:   inc     [Seg64]                 ; Segment-�berlauf ber�cksichtigen
        call    [SegSelect]             ; Segment setzen
        stosb                           ; Punkt setzen
...
; -------------------------------------------------------------------
; Unterprogramm f�r Ellipse zum Setzen des 4-fach gespiegelten Punktes mit
; 1 Pixel Durchmesser.
; cx = x, bx = y, ds = DSeg, es = VideoSeg
; ds, es und bp m�ssen bleiben alles andere darf zerst�rt werden.
...
; X-Werte berechnen und gegen die Clipping-Fenstergrenzen pr�fen
...
; um den Betrag den X1 damit vergr��ert wurde, vermindert. Damit wird X2
; korrekt geplottet und die Adress-Berechnung stimmt ohne 10-zeilige
; Assembler Klimmz�ge in der innersten Schleife.
...
; Y-Wert der oberen beiden Punkte berechnen, pr�fen, ob diese Punkte im Fenster
; liegen
...
; Pr�fen, ob einer der beiden Radien <= 0 ist. Wenn ja, direkt Ende
...
; Sonstiger Kleinkruscht
        ...
        mov     es, [VideoSeg]
        mov     [Seg64], 0FFh           ; Flag f�r "kein Segment gesetzt"
        mov     cx, [X]
        mov     bx, [Y]
        call    [PlotVector]
...
; Was sonst noch in der ersten Region ausgef�hrt werden mu�
...
; ---------------------------------------------------------------------------
; Hauptprogramm f�r Ellipse. Wird �ber den Dispatcher aufgerufen.
...
; Pr�fen ob es sich um eine 360� Ellipse handelt
...
; Es handelt sich um eine 360 Grad-Ellipse.  Liniendicke pr�fen und
...
; schneller.
; Anzahl nach cx und pr�fen, ob innerhalb der Linie ein Segment-�berlauf
; stattfindet.
...
; Es findet ein �berlauf statt. Die Linie in zwei Teilen zeichnen

        mov     bx, di
        neg     bx                      ; Anzahl bis �berlauf
...
; N�chstes Segment setzen
        ...
        inc     [Seg64]                 ; N�chstes Segment...
        call    [SegSelect]             ; ...setzen
        mov     cx, bx                  ; Restliche Anzahl

; Falls die restliche Anzahl 0 ist passiert hier nix, weil rep dann 0 mal
; ausgef�hrt wird (ganz im Gegensatz zu weiter unten, loop wird n�mlich dann
; 65536 mal ausgef�hrt ... wie wir sie lieben, die Welt der Kompatiblen).
...
; Es wird mit einem Muster gef�llt. Das Muster befindet sich in ah und mu�
; f�r die Startkoordinate passend gemacht werden.
...
EVEN
@@L4:   mov     [Byte es:di], 0         ; Pixel in Hintergrundfarbe f�llen
        inc     di
        loopcx  @@L3
@@L5:
...
EVEN
@@L8:   mov     [Byte es:di], 0         ; Pixel in Hintergrundfarbe f�llen
        inc     di
        loopcx  @@L7
...
; -------------------------------------------------------------------
; Unterprogramm f�r FilledEllipse zum Zeichnen der Ellipse.
;
...
; ----------------------------------------------------------------------
;
; Unterprogramm zum Zeichnen einer gef�llten Ellipse. Es wird Ellipse mit
...
; Die folgende Struktur _mu�_ zuallererst im Datensegment liegen !
                ...
                dw      OFFSET Segs
                db      1               ; Ein Int-Vektor wird ben�tigt
                dw      OFFSET Ints
...
LABEL           LowSegs
                dw      16              ; Gr��e: 16 Paras (256 Bytes)
LowBufSeg       dw      0               ; Real-Mode Segment
LowBufSel       dw      0               ; Protected Mode Selector
ELSE

; Statischer Puffer im Low Memory f�r Versionen vor 3.0
LowBuf          db      256 dup (?)
...
;---------------------------------------------------
; Vektor-Tabelle
; Bemerkung zur Tabelle: Nach c't 11/89, 9/90 und 2/91 enth�lt der Vektor
; 14 (0Eh) einen Einsprung f�r den Scan-Konverter von FillPoly. Dieser Vektor
; wird jedoch nicht eingetragen, da das Grafik-Kernel von BP 7.0 diesen
; Vektor pr�ft und wenn dieser nicht mehr auf Emulate zeigt, ein Flag setzt,
; was zur Folge hat, das Arc und PieSlice mit kleinen Radien nicht mehr
; korrekt arbeiten. Also steht der Vektor hier auf Emulate und wird sp�ter
...
SegSelect       dw      NOP_Vector      ; Vektor f�r Segment-Umschaltung
PlotVector      dw      EPlot4Thin      ; Zeichenvektor f�r DoEllipse
HorLine         dw      Generic_HorLine ; Horizontale Linie im aktuellen F�llmuster
HorLineInit     dw      NOP_Vector      ; Init f�r HorLine
...
;---------------------------------------------------
; Flags f�r Optionen. Siehe die entsprechenden Konstanten in const.asi

Options         dw      0
OptText         db      9, "SVGAOPTS="

;---------------------------------------------------
; Gr��e des Bildschirms in Bytes als LongInt (f�rs L�schen)

ScreenBytes     dd      0

;---------------------------------------------------
; X- und Y-Aufl�sung des gerade aktiven Modus und Y-Offset der aktuelle
; Seite.
...
; wurde mit MaxX gerechnet), k�nnen bei VESA-Treibern aber auch unter-
; schiedlich (gr��er) sein.
...
;---------------------------------------------------
; Hier stehen die Farben: Hintergrundfarbe, Zeichen- und F�llfarbe
;

BkColor         db      00h             ; Kommt nach Palettenregister 0
DrawingColor    db      0Fh             ; Zeichenfarbe
FillColor       db      0Fh             ; F�llfarbe
...
;---------------------------------------------------
; Die Default-Farbtabelle enth�lt 16 Eintr�ge:

ColorTable      db      10h     ; 16 Eintr�ge folgen
...
;---------------------------------------------------
; Diverse Informationen f�r den Hardware-Font
...
;---------------------------------------------------
; LinePattern (Pattern f�r gesetzten LineStyle) und Linienbreite

LinePattern     dw      0FFFFh          ; SolidLn
LineWidth       dw      1

;---------------------------------------------------
; Tabelle der Bitmuster f�r die Linestyles.
...
; Der letzte Eintrag ist f�r das User-Pattern freigehalten, das hierher
; kopiert wird.
...
; f�r den Autodetect und die Adresse des Extended-Registers der ATI-Wonder
;
...
VESA_Granularity        db      1       ; Granularit�t des Fensters
EVEN
...
;---------------------------------------------------
; Die Namen der verschiedenen Modi. Auf die Namen wird �ber eine Tabelle
...
; Tabelle mit den Mode-Bits f�r die Autodetect-Modi
...
; --------------------------------------------------------------
; Die Segment-Select-Routinen f�r die verschiedenen Karten.
...
LABEL SegSwitchTable    WORD
        ...
        dw      VESA_SegSwitch2         ; VGA unterst�tzt VESA-Standard

; --------------------------------------------------------------
; SetDrawPage f�r die verschiedenen Karten.
; Die Tabelle wird direkt mit der Karten-Nummer indiziert.
...
LABEL   SetDrawPageTable        WORD
        ...
        dw      Generic_DrawPage        ; VGA unterst�tzt VESA-Standard

; --------------------------------------------------------------
; SetVisualPage f�r die verschiedenen Karten.
; Die Tabelle wird direkt mit der Karten-Nummer indiziert.
...
LABEL   SetVisualPageTable      WORD
        ...
        dw      VESA_VisualPage         ; VGA unterst�tzt VESA-Standard

; ------------------------------------------------------------
;
; Tabellen mit den Werten der Betriebsarten. Aus Platzgr�nden und weil es
; Vorteile hat, genau *eine* DST zu haben, sind die Werte nicht wie fr�her
...
; -----------------------------------------------
; Der Status-Record, der an TP �bergeben wird.
